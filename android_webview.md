...
*   **File Access Issues:** WebView accessing local files (`file:///android_asset/`, `file:///android_res/`, app's private data directories via `file://`) it shouldn't, or web content accessing sensitive files via WebView APIs or bugs.
    *   **VRP Pattern:** Incorrect handling of `file://` URLs (lack of `AllowFileAccessFromFileURLs` or `AllowUniversalAccessFromFileURLs` checks in certain paths) or specific headers potentially allowing access to sensitive app files. (VRP: `1301873` related issue - `android_webview_app_defined_websites.md` likely connects here). Check how `AwSettings.java` maps to native preferences and if they are consistently checked.
...
*   **Multiple Window Support:** Analyze the code paths taken when `setSupportMultipleWindows` is `false` (default) vs. `true`. Ensure the `false` path doesn't introduce vulnerabilities like the `javascript:` URL SOP bypass (VRP: `40052335`).
...
*   `AwWebContentsDelegate.java`: Handles requests initiated by web content (e.g., opening new windows - `onCreateWindow`). **Crucial for handling `window.open` when multiple windows are disabled.**
