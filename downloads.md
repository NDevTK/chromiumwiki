...
*   **File Type/Extension Handling & Path Validation:** Bypassing checks intended to warn about or block dangerous file types, or allowing downloads to unintended locations. This is often linked to insufficient sanitization of filenames, especially those suggested by extensions or manipulated in file dialogs.
...
*   **Sandbox Bypass:** Downloads initiated from sandboxed contexts bypassing restrictions, or downloaded files leading to execution outside the sandbox.
    *   **VRP Pattern (iframe `allow-downloads`):** Bypassing the lack of `allow-downloads` sandbox flag (VRP: `40060695`, VRP2.txt#11682 - potentially related to `.xht` handler). See [iframe_sandbox.md](iframe_sandbox.md).
    ...
    *   **VRP Pattern (Drag-and-Drop `file:` URL):** Dropping a `file:` URL that passes initial MIME filter (`FilterURLsForDropability` checks `blink::IsSupportedMimeType`) might lead to RCE if the handler for that allowed file type (e.g., `.xht` VRP#11682) has a vulnerability exploitable from a local file context.
...
*   **Sandbox Interactions:** Verify that downloads initiated from sandboxed iframes correctly adhere to the `allow-downloads` flag in all cases (including `noopener` popups - VRP2.txt#1105). Verify the process handling `file:` URL navigations initiated by drag-and-drop.
...
*   **XHTML/File Handler Vulnerabilities:** Investigate potential vulnerabilities in Blink's handling of XHTML or other file types allowed through drag-and-drop filtering (VRP#11682).
*   **Save As Dialog Behavior:** Deep dive into `IFileSaveDialog` behavior regarding user input vs. selected filters for extension handling (related to VRPs `1228653`, `1378895`).
...
*(See also [safe_browsing_service.md](safe_browsing_service.md), [extensions_api.md](extensions_api.md), [file_system_access.md](file_system_access.md), [iframe_sandbox.md](iframe_sandbox.md), [drag_drop.md](drag_drop.md))*
