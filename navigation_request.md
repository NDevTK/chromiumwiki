...
-   `NavigationRequest::MaybeRecordTraceEventsAndHistograms`: Records trace events and histograms.
-   `NavigationRequest::ResetViewTransitionState`: Resets the view transition state.
-   `NavigationRequest::IsDisabledEmbedderInitiatedFencedFrameNavigation`: Checks if the navigation is a disabled embedder-initiated fenced frame navigation.
-   `NavigationRequest::SetExpectedProcessIfAssociated`: Sets the expected process to the process of the current associated RFH.
-   `NavigationRequest::SanitizeDocumentIsolationPolicyHeader`: Sets the Document-Isolation-Policy header to a default value in unsecure contexts or if DocumentIsolationPolicy is not supported.
-   `NavigationRequest::CheckPermissionsPoliciesForFencedFrames`: Checks that the required permissions policy features are enabled for fenced frames.
-   `NavigationRequest::IsFencedFrameRequiredPolicyFeatureAllowed`: Checks if a specific permissions policy feature is allowed for fenced frames.
-   `NavigationRequest::GetUserAgentOverride`: Returns the user-agent override, or an empty string if one isn't set.
-   `NavigationRequest::CreateCookieAccessObserver`: Creates a CookieAccessObserver for the navigation.
-   `NavigationRequest::OnCookiesAccessed`: Called when cookies are accessed during the navigation.
-   `NavigationRequest::Clone`: Clones a CookieAccessObserver.
-   `NavigationRequest::CreateTrustTokenAccessObserver`: Creates a TrustTokenAccessObserver for the navigation.
-   `NavigationRequest::OnTrustTokensAccessed`: Called when Trust Tokens are accessed during the navigation.
-   `NavigationRequest::CreateSharedDictionaryAccessObserver`: Creates a SharedDictionaryAccessObserver for the navigation.
-   `NavigationRequest::OnSharedDictionaryAccessed`: Called when a shared dictionary is accessed during the navigation.
-   `NavigationRequest::CreateDeviceBoundSessionObserver`: Creates a DeviceBoundSessionAccessObserver for the navigation.
-   `NavigationRequest::OnDeviceBoundSessionAccessed`: Called when a device-bound session is accessed during the navigation.
-   `NavigationRequest::GetNavigationController`: Returns the NavigationControllerImpl for this navigation.
-   `NavigationRequest::GetPrerenderHostRegistry`: Returns the PrerenderHostRegistry for this navigation.
-   `NavigationRequest::GetInitiatorDocumentRenderFrameHost`: Returns the RenderFrameHost of the initiator document, if available.
-   `NavigationRequest::RecordAddressSpaceFeature`: Records the appropriate kAddressSpace* WebFeature for the response.
-   `NavigationRequest::ComputePoliciesToCommit`: Computes the PolicyContainerPolicies and sandbox flags for committing a regular document.
-   `NavigationRequest::ComputePoliciesToCommitForError`: Computes the PolicyContainerPolicies and sandbox flags for committing an error document.
-   `NavigationRequest::MakeDidCommitProvisionalLoadParamsForBFCacheRestore`: Builds the parameters for committing a navigation to a page restored from the back-forward cache.
-   `NavigationRequest::MakeDidCommitProvisionalLoadParamsForPrerenderActivation`: Builds the parameters for committing a navigation to a prerendered page that was activated.
-   `NavigationRequest::MakeDidCommitProvisionalLoadParamsForActivation`: Builds generic activation parameters for committing a navigation to a page.
-   `NavigationRequest::CheckCSPDirectives`: Checks if CSP allows the navigation.
-   `NavigationRequest::CheckContentSecurityPolicy`: Checks if the request should be allowed to continue or should be blocked due to CSP.
-   `NavigationRequest::ShouldReplaceCurrentEntryForSameUrlNavigation`: Determines if a same-URL navigation should replace the current entry.
-   `NavigationRequest::ShouldReplaceCurrentEntryForNavigationFromInitialEmptyDocumentOrEntry`: Determines if a navigation from an initial empty document or entry should replace the current entry.
-   `NavigationRequest::ShouldReplaceCurrentEntryForFailedNavigation`: Determines if a failed navigation should replace the current entry.
-   `NavigationRequest::GetOriginForURLLoaderFactoryBeforeResponseWithDebugInfo`: Same as `GetOriginForURLLoaderFactoryBeforeResponse`, but includes debug info.
-   `NavigationRequest::GetOriginForURLLoaderFactoryAfterResponseWithDebugInfo`: Same as `GetOriginForURLLoaderFactoryAfterResponse`, but includes debug info.
-   `NavigationRequest::ComputeCrossOriginIsolationKey`: Computes the CrossOriginIsolationKey for the navigation.
...
-   The logic for determining the `UrlInfo` for different types of navigations.
-   The handling of redirects and cross-origin navigations.
-   The interaction between `NavigationRequest` and other security mechanisms, such as CSP and COOP.
-   The impact of incorrect origin handling on cross-origin communication and data access.
-   The details of the navigation state transitions and their implications for security.
-   The process of selecting the appropriate `RenderFrameHost` for a navigation.
-   The role of `NavigationHandle` and its interaction with `NavigationRequest`.
-   The handling of the cancellation window for renderer-initiated navigations.
-   The mechanisms for handling failed navigations and error pages.
-   The usage of `NavigationRequest` in special cases like data URLs, about:blank, and downloads.
-   The interaction between `NavigationRequest` and features like prerendering, back/forward cache, and fenced frames.
-   The role of `NavigationRequest` in the context of service workers and web workers.
-   The implications of different `NavigationType` values on the navigation process.
-   The usage of `NavigationRequest` in the context of WebUI pages.
-   The handling of `CrossOriginIsolationKey` and its impact on site isolation.
-   The role of `NavigationRequest` in the context of the Origin-Agent-Cluster header.
-   The mechanisms for handling cookie access, Trust Tokens, and shared dictionaries during navigation.
-   The interaction between `NavigationRequest` and the `SiteInstanceGroupManager`.
-   The usage of `NavigationRequest` in the context of subframe navigations and the navigate event.
